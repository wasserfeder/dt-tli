...
from stl_prim import set_stl2_pars, SimpleModel, split_groups
from stl_syntax import robustness
import numpy as np
import math


def optimize_inf_gain(traces, primitive, rho, pdist, disp=False):
    """
    ...
    args = (primitive, models, rho, traces, maxt, pdist)
...
def _inweights(part, stotal):
    if len(part) == 0:
        return 0
    return sum(np.multiply(np.abs(zip(*part)[0]), zip(*part)[2])) / stotal

def _entropy(part):
    if len(part) == 0:
        return 0

    spart = float(sum(np.multiply(np.abs(zip(*part)[0]), zip(*part)[2])))
    ...
    if spart == 0:
        ...
        w_p = sum([abs(p[0])*p[2] for p in part if p[1] >= 0]) / spart
        w_n = sum([abs(p[0])*p[2] for p in part if p[1] < 0]) / spart

...
from stlprim import make_stl_primitives1, make_stl_primitives2
import numpy as np
...
def bdt_stlinf_(traces, rho, depth, pdist, inc, optimize_impurity, stop_condition, disp=False):
    args = locals().copy()
    # Stopping condition
    ...
    primitives2 = make_stl_primitives2(traces.signals)
    # primitives = make_llt_primitives(traces.signals)
    #primitives = make_stl_primitives(traces.signals)
    primitive, impurity = _find_best_primitive(traces, primitives1, primitives2, rho, pdist,
...
def _find_best_primitive(traces, primitives1, primitives2, robustness, pdist, optimize_impurity, disp):
    # Parameters will be set for the copy of the primitive
    opt_prims = []
    for primitive in primitives1:
        opt_prims.append(optimize_impurity(traces, primitive.copy(), True, robustness, pdist, disp))

    for primitive in primitives2:
        opt_prims.append(optimize_impurity(traces, primitive.copy(), False, robustness, pdist, disp))

...
def pso_costFunc(position, signals, labels, index, rho_path):
    [pi, t0, t1] = position
    if t0 >= t1:
        print("Wrong Input")
        return
    ...
    rhos = [compute_robustness(signals[i], pi, int(t0), int(t1), index, rho_path[i]) for i in range(len(signals))]
...
class Particle():
    def __init__(self, x0, v0, signals, labels, bounds, index):
        self.position = x0
        self.velocity = v0
    ...
    def evaluate(self, costFunc, rho_path):
        self.err_i = costFunc(self.position, self.signals, self.labels, self.index, rho_path)

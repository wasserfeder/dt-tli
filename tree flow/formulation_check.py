...
class FlowMILP(object):
    ...
    def __init__(self, signals, labels, ranges, rho_path, signal_dimension=0, depth=5, model=None):
        ...
        # Decision Variables
        '''
        self.b = 0   # binary, b[n] == 1 :if branching at nodde n [cannot be labelled]
        ...
        self.z_phi = self.model.addVars(self.signal_index, self.tree.Nodes, vtype=GRB.BINARY, name='z_phi')
        ...
        # for current_node in self.tree.Nodes:
        #     for i in range(len(self.signals)):
        #         rho[i] = self.robustness(i)
        #         self.model.addConstr(self.z_phi[i,current_node]*self.M >= rho[i])
        #         self.model.addConstr(-(1-self.z_phi[i,current_node]) * self.M <= rho[i])


        # Ensure branching occurs iff a signal satisfies a particular primitive
        # self.model.addConstr(self.z[])

...
def inc_tree(tr_s, tr_l, te_s, te_l, rho_path, primitives, D_t, args):
    ...
    return trees, formulas, weights, 100 * tr_MCR, 100 * te_MCR, 100 * tr_MCR_vector, 100 * te_MCR_vector
...
def single_tree(tr_s, tr_l, te_s, te_l, rho_path, primitives, D_t, args):
    ...
    if not args.inc_eval:
        return tree, formula, None, tr_MCR, te_MCR, None, None
...
def learn_formula(tr_s, tr_l, te_s, te_l, args):
    ...
    if not inc:     # Non-incremental version
        if numtree == 1:   # No boosted decision trees
            tree, formula, weight, tr_MCR, te_MCR, tr_MCR_vector, te_MCR_vector = single_tree(tr_s, tr_l, te_s, te_l,
                                    rho_path, primitives, D_t, args)

        else:
            tree, formula, weight, tr_MCR, te_MCR, tr_MCR_vector, te_MCR_vector = boosted_trees(tr_s, tr_l, te_s, te_l,
                                    rho_path, primitives, D_t, args)
    else:         # Incremental version
        tree, formula, weight, tr_MCR, te_MCR, tr_MCR_vector, te_MCR_vector = inc_tree(tr_s, tr_l, te_s, te_l, rho_path,
                                            primitives, D_t, args)



    return tree, formula, weight, tr_MCR, te_MCR, tr_MCR_vector, te_MCR_vector
...
def kfold_learning(filename, args):
    ...
    if k_fold <= 1:     # No cross-validation
        tree, formula, weight, tr_MCR, te_MCR, tr_MCR_vector, te_MCR_vector = learn_formula(signals, labels, None,None,args)
        trees.append(tree)
        print('***************************************************************')
        ...
        for train_index, test_index in kf.split(signals):
            ...
            tree, formula, weight, tr_MCR, te_MCR, tr_MCR_vector, te_MCR_vector = learn_formula(tr_signals, tr_labels,te_signals, te_labels, args)
            trees.append(tree)
    ...
    for tree in trees:
        tree_args = {'pose': 'root', 'parent_x': None, 'parent_y': None}
        treefig, ax = plt.subplots(1,1)
        draw_tree(tree, tree_args, ax)
        ax.autoscale_view()
        plt.xlim([-5,15])
        plt.ylim([0,15])
        plt.show()
            ...
            for k_max in candidate_k_max:
                for num_particles in candidate_num_particles:
                    ...
                    for train_index, test_index in kf.split(signals):
                        ...
                        te_signals  = np.array([signals[i] for i in test_index])
                        te_labels   = np.array([labels[i] for i in test_index])
                        tree, formula, weight, tr_MCR, te_MCR = learn_formula(tr_signals, tr_labels,

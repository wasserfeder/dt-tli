...
# ------------------------------------------------------------------------------
# ==============================================================================
class Non_Inc_Build_Tree(object):
    ...
    def build_tree(self, signals, labels, rho_path, depth, D_t, parent):
        if (depth <= 0) or (len(signals) == 0):
            return None

        prim, impurity, rhos = self.best_prim_pso(signals, labels, rho_path, D_t)
        ...
        tree.left = self.build_tree(sat_signals, sat_labels, sat_rho, depth - 1,
                    sat_weights, tree)
        right_tree = tree
        right_tree.primitive.reverse_op()
        right_tree.primitive.reverse_rel()
        tree.right = self.build_tree(unsat_signals, unsat_labels, unsat_rho, depth - 1,
                    unsat_weights, right_tree)

        return tree


    def best_prim_pso(self, signals, labels, rho_path, D_t):
        opt_prims = []
